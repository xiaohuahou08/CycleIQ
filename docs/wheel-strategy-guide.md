...
## 7) CycleIQ Screener（选股扫描，仅供参考）

CycleIQ 的 **Screen / 选股** 页可按可配置规则扫描 Sell Put（观察列表）与 Covered Call（已指派持股）候选：

- 门槛与收益一律按 **每股美金（$/share）** 计算，不是整张 ×100。  
- 行情与期权链来自 Yahoo Finance（yfinance）；缺数据则跳过该合约。  
- 结果为 **advisory-only**：帮助人工筛选，**不会自动下单**，也不构成投资建议。  
- 与交易记账单、Wheel 周期账本分离——扫描不会写入 trades / cycles。
